...
namespace MetroRx
{
    ...
    public static class EnableLoggerMixin
    {
        static MemoizingMRUCache<int, ILog> loggerCache = new MemoizingMRUCache<int, ILog>(
            (_, obj) => { var t = obj.GetType().GetTypeInfo(); return RxApp.LoggerFactory(t.FullName); }, 50);
    ...
    public abstract class LoggerBase : ILog
    {
        ...
        static LoggerBase()
        {
            ...
            stringFormat = ti.GetDeclaredMethods("Format")
                .First(x => { var p = x.GetParameters(); return (p.Length == 3 && p[0].ParameterType == typeof(IFormatProvider)); });
        }
    ...
    public class StdErrLogger : LoggerBase
    {
        ...
        {
            lock (gate) { DiagDbg.WriteLine("FATAL ERROR: ******" + message + " ******"); }
        }
    }
}
